"""Zhipu GLM provider through its OpenAI-compatible endpoint."""

from __future__ import annotations

import os

from ..contracts import ModelCapability
from .openai_compatible import OpenAICompatibleProvider, ProviderSettings


class GLMProvider(OpenAICompatibleProvider):
    def __init__(self, *, transport=None) -> None:
        model = os.environ.get("GLM_MODEL", "glm-4-plus")
        super().__init__(
            ProviderSettings(
                provider_id="glm",
                model_id=model,
                base_url=os.environ.get(
                    "GLM_BASE_URL", "https://open.bigmodel.cn/api/paas/v4"
                ),
                api_key=os.environ.get("GLM_API_KEY", ""),
            ),
            ModelCapability(
                provider_id="glm", model_id=model, region="CN",
                tool_calling=True, structured_output=True,
                context_tokens=int(os.environ.get("GLM_CONTEXT_TOKENS", "128000")),
                adapter_version="1.0",
            ),
            transport=transport,
        )
