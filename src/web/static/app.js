...
function bindEvents() {
  ...
  $('#scrim').addEventListener('click', () => { closeDrawer(); $('#sidebar').classList.remove('open'); });
...
async function startRecovery(mode, runId) {
  ...
  try {
    const endpoint = dataRetry ? '/api/actions/retry-tier1-data' : '/api/actions/resume-tier1';
    const response = await fetch(endpoint, {method:'POST', headers:{'Content-Type':'application/json'}, body:JSON.stringify({run_id:runId})});
...
async function submitReview(event) {
  ...
  try {
    const response = await fetch(`/api/reviews/stage-${stage.toLowerCase()}`, { method: 'POST', headers: {'Content-Type':'application/json'}, body: JSON.stringify(payload) });
...
async function exportTier2(symbols) {
  try {
    const response = await fetch('/api/actions/export-tier2', { method:'POST', headers:{'Content-Type':'application/json'}, body:JSON.stringify({run_id:state.data.run.run_id, symbols}) });
    const data = await response.json(); if (!response.ok) throw new Error(data.error || '启动失败');
...
function navigate(page) {
  ...
  const labels = { overview:'研究总览', candidates:'候选雷达', quality:'数据质量', runs:'运行记录' }; $('#pageTitle').textContent = labels[page];
